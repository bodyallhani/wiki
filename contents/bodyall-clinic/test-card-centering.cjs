...
ctx.fillText=(text,x,y)=>{if(text==='© Bodyall')label={x,y,width:ctx.measureText(text).width,ascent:ctx.measureText(text).actualBoundingBoxAscent,descent:ctx.measureText(text).actualBoundingBoxDescent};return originalText(text,x,y);};
...
assert.equal(label.x-mark.x-mark.w,12);

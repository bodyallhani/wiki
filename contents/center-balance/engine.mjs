...
export class TowerGame {
  ...
  reset() {
    ...
    this.baseSlope = slopeDirection * (.018 + this.random() * .016);
    this.time = 0;
    this.state = 'ready';
    this.reason = null;
    this.level = 0;
  ...
  spawnMover() {
    ...
    const width = Math.max(1.72, 2.58 - this.level * .018);
    this.mover = {
      x: direction * 3.45,
      direction: -direction,
      width,
      speed: Math.min(3.55, 1.52 + this.level * .075),
  ...
  structuralLean(blocks = this.blocks) {
    ...
    return clamp(this.baseSlope + center * .082 + localKink * .034, -.31, .31);
  }
  ...
  idealX() {
    ...
    const desired = -(weightedX + this.baseSlope / .082 * Math.max(1, totalWeight)) / newWeight;
    const reach = (support.width + this.mover.width) * .5 - .42;
    return clamp(desired, support.x - reach, support.x + reach);
  }
  ...
  place() {
    ...
    if (this.level < 3 && Math.abs(x - ideal) < .24) x = ideal;
    ...
    const perfect = accuracy < .095;
    if (perfect) { this.perfects++; this.combo++; }
    else this.combo = 0;
    const recovery = before >= .5 && projected <= before - .23;
  ...
  step(dt) {
    ...
    if (instant > .19) this.strain += (instant - .16) * dt * .19;
    else this.strain -= dt * .12;
    this.strain = clamp(this.strain, 0, 1);
    this.previousLoad = this.load;
    this.load = clamp(instant * .69 + this.strain * .64, 0, 1.15);
    this.overloadTime = this.load > .78 ? this.overloadTime + dt : Math.max(0, this.overloadTime - dt * 1.8);
    this.maxLoad = Math.max(this.maxLoad, this.load);
    this.maxLean = Math.max(this.maxLean, Math.abs(this.lean));
    this.score += dt * (8 + this.level * .45);
    if (this.load >= MAX_LOAD || this.overloadTime > 2.15) this.finish('overload');
  ...
  snapshot() {
    return {
      ...
      overloadTime: this.overloadTime, baseSlope: this.baseSlope,
